...
const incoming = new Set(['RECEIVED', 'TRANSFER_IN', 'OPENING']);
...
export async function GET(req: Request) {
  try {
    ...
    const rows = items.map((item) => {
      ...
      const openingMovements = movements.filter((movement) => movement.movementCategory === 'OPENING');
      const openingOverride = openingMovements.length ? openingMovements.reduce((total, movement) => total + Math.abs(movement.quantity), 0) : undefined;
      const openingCorrection = movements.filter((movement) => movement.movementCategory === 'RECONCILIATION' && String(movement.reason || '').toLowerCase().includes('opening correction')).reduce((total, movement) => total + movement.quantity, 0);
      ...
      movements.forEach((movement) => {
        ...
        if (movement.movementCategory === 'RECONCILIATION' && !String(movement.reason || '').toLowerCase().includes('opening correction')) draft.misExt = Number(draft.misExt || 0) + (movement.quantity >= 0 ? quantity : -quantity);
        if (movement.comment) draft.comment = movement.comment;
      });
      return { itemId: item.id, opening: (openingOverride ?? openingByItem.get(item.id) ?? item.quantityOnHand) + openingCorrection, draft };
    });
